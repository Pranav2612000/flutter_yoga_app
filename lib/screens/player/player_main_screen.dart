...
class _PlayerMainScreenContent extends StatefulWidget {
  ...
  @override
  _PlayerMainScreenContentState createState() =>
      _PlayerMainScreenContentState();
}
  ...
  void initState() {
    ...
    _asanasInQueueBarAutoScrollReaction = reaction(
      ...
      (index) {
        ...
        final hash =
            _getHashForAsanaInQueueBar(store.currentAsanaUniqueName, index);
          ...
          decoration: BoxDecoration(
            border: Border(
              bottom: BorderSide(
                width: 3,
                color:
                    isActive ? Styles.accentGreyColor : Styles.shadedGreyColor,
              ),
            ),
          ),
          child: Opacity(
  ...
  Widget _currentAsana() {
    return Container(
      ...
      child: Consumer<AsanasStore>(builder: (_, asanasStore, __) {
        return Observer(builder: (_) {
          if (store.currentAsanaUniqueName == null ||
              store.currentAsanaUniqueName.isEmpty) {
            return SizedBox.shrink();
          }
  ...
  Widget _currentPlayerItem(BuildContext context) {
    ...
    if (store.currentQueueItem.isChilling) {
      ...
      if (store.nextQueueItemWithAsana != null) {
        final asanasStore = Provider.of<AsanasStore>(context, listen: false);
        nextAsana =
            asanasStore.asanas[store.nextQueueItemWithAsana.asanaUniqueName];
...
//                  : Styles.timerPauseColor,
  ...
  Widget _rewindButton() {
    return Observer(builder: (_) {
      final onTap = store.isRewindEnabled
          ? () => store.rewindCurrentPhaseOrGoBack()
          : null;
